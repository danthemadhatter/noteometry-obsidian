...
- Create a GitHub release whose tag matches the version number. **This repo uses the `v` prefix** (e.g. `v1.7.2`) — the `.github/workflows/main.yml` release workflow fires on any pushed tag and uses `github.ref_name` directly for the release name and tag, so BRAT users must see a release with exactly `v1.7.2` published for `manifest.json` version `1.7.2` to pull through. (Obsidian's own sample plugin guideline uses a bare number; this project diverged for compatibility with the existing release history — do not change the convention without migrating every prior tag.)
